...
const StatusBadge = ({ status, className = "" }) => {
  ...
  return (
    <span
      className={`px-4 py-1.5 backdrop-blur-md rounded-full text-[10px] font-black uppercase tracking-widest border transition-all ${config[status] || config.CANCELLED} ${className}`}
    >
      {status.replace("_", " ")}
    </span>
  );
